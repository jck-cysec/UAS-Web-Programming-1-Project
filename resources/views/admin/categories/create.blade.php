    ...
    <div class="relative h-[300px] bg-[#1a1a1a]">
        ...
        <div class="relative h-full max-w-7xl mx-auto px-6 md:px-12 flex items-end pb-8">
            <div class="space-y-2 max-w-3xl">
                <h1 class="text-3xl md:text-5xl font-black text-white leading-tight">Create New Category</h1>
                <p class="text-base md:text-lg text-white/90 font-medium">Add a new game category to organize your store</p>
            </div>
        </div>
    ...
    <div class="bg-[#E8DCC4] rounded-2xl border-4 border-black p-4 sm:p-6 md:p-8 shadow-lg">
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-4 pt-6 border-t-2 border-black">
                <button type="submit" class="rounded-full bg-black text-white px-6 py-3 font-black uppercase tracking-wide hover:scale-105 shadow-lg transition-transform text-sm w-full sm:w-auto">
                    Create Category
                </button>
                <a href="{{ route('admin.categories.index') }}" class="rounded-full bg-white border-2 border-black text-black px-6 py-3 font-black uppercase tracking-wide hover:scale-105 shadow-lg transition-transform inline-flex items-center justify-center text-sm w-full sm:w-auto">
                    Cancel
                </a>
            </div>
        </form>
